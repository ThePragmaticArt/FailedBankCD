...
-(NSFetchedResultsController *)fetchedResultsController{
    ...
    NSSortDescriptor *sort = [[NSSortDescriptor alloc] initWithKey:@"details.closeDate" ascending:NO]; //sorts by close date in the FailedBankDetails, but still only receive the data in FailedBankInfo
    [fetchRequest setSortDescriptors:[NSArray arrayWithObject:sort]];
    [fetchRequest setFetchBatchSize:20];
    NSFetchedResultsController *theFetchedResultsController = [[NSFetchedResultsController alloc] initWithFetchRequest:fetchRequest managedObjectContext:managedObjectContext sectionNameKeyPath:nil cacheName:@"Root"]; //The section name key path lets us sort the data into sections in our table view. We could sort the banks by State if we wanted to, for example, here.
...
- (void)viewDidLoad
{
    ...
    NSError *err;
    if (![[self fetchedResultsController] performFetch:&err]) { //recreates controller as well do to the viewDidUnload setting it to nil. Saves memory
...
#pragma mark Delegate methods for the NSFetchedResultsController
